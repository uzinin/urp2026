...
class Yolov8InfoExtractor(Node):
    ...
    def yolov8_detections_callback(self, detection_msg: DetectionArray):
        ...
        if lane2_edge_image is not None and lane2_edge_image.size > 0:
            try:
                lane2_bird_image = CPFL.bird_convert(lane2_edge_image, srcmat=src_mat, dstmat=dst_mat)
                
                # (이후 lane2_bird_image를 사용하는 코드들도 이 if문 안으로 넣거나,
            ...
            self.get_logger().warn("Skipping frame: 'lane2_edge_image' is empty or invalid.")
            return
        # lane2_bird_image = CPFL.bird_convert(lane2_edge_image, srcmat=src_mat, dstmat=dst_mat)
